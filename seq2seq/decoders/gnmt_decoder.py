...
class GNMTDecoder(RNNDecoder):
    ...
    def __init__(self,
        ...
        super(GNMTDecoder, self).__init__(params, mode, name)
    ...
    def step(self, time_, inputs, state, name=None):
        attention_cell = self.cell[0]
        attention_state = state[0]
        cell_output, new_attention_state = attention_cell(inputs, attention_state)
        ...
        sample_ids = self.helper.sample(
            time=time_, outputs=logits, state=new_attention_state)
        ...
        finished, next_inputs, next_state = self.helper.next_inputs(
            time=time_, outputs=outputs, state=new_attention_state, sample_ids=sample_ids)
